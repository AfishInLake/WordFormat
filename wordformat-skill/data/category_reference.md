...
| `heading_mulu` | 段落等于"目录"或"目 录"（含空格变体） | "目录"、"目 录" |
...
| 内容类型 | 典型示例 | 说明 |
|----------|----------|------|
| 目录标题 | "目 录"、"目录" | 改为 `"heading_mulu"`，代码会自动跳过 |
...
处理规则：
1. **目录标题**（如"目录"、"目 录"）→ 改为 `"heading_mulu"`
2. **所有目录条目**（如"1 绪论 ...... 1"、"2.1 研究背景 ...... 5"）→ 改为 `"other"`
3. **目录区域内的其他段落**（如空行、分隔线等）→ 改为 `"other"`
...
- 目录标题标记为 `"heading_mulu"` 后，代码会自动跳过该节点及其所有子节点的格式化
...
JSON 中每个段落条目可以包含一个可选的 `"replace"` 字段，用于在格式化时直接替换段落文本内容。

**使用场景**：
- AI 识别文字有误（如 OCR 错误、空格异常）：`"摘    要"` → `"摘  要"`
- 修正错别字或特定短语
- 更正标题文字

**用法**：直接在 JSON 条目的 `replace` 字段填入新文本即可：
```json
{
...
**注意事项**：
- `replace` 是可选字段，不需要时不用添加
- 替换在格式化之前执行，替换后的文本会被按配置规范格式化
- 检查和格式化模式均会执行替换
- 不要用 `replace` 来修改 `category`——修改分类请直接改 `category` 字段
