...
`tables.content` 控制表格单元格内文字的格式，继承 `GlobalFormatConfig` 的 15 个字段。在格式化模式（`wordf af`）下，工具会遍历所有表格的所有单元格段落，应用此配置。
...
控制标题和参考文献的自动编号功能，包括编号格式、编号与文字的间距、缩进等。**仅在格式化模式（`wordf af`）下生效**，检查模式（`wordf cf`）不会修改编号。
