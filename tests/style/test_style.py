...
def test_paragraph_get_space_before():
    ...
    # 测试默认情况
    assert paragraph_get_space_before(paragraph) == 0.0
...
def test_paragraph_get_space_after():
    ...
    # 测试默认情况
    assert paragraph_get_space_after(paragraph) == 0.0
...
def test_set_first_line_indent_char():
    ...
    with patch.object(_SetFirstLineIndent, 'clear') as mock_clear:
        ...
        with patch('wordformat.style.set_some.parse_xml') as mock_parse_xml:
            ...
            # 执行函数
            result = _SetFirstLineIndent.set_char(paragraph, 2.0, "char")
        ...
        with patch('wordformat.style.check_format.paragraph_get_space_before', return_value=0.5):
            ...
            with patch('wordformat.style.check_format.paragraph_get_space_after', return_value=0.5):
                ...
                with patch('wordformat.style.check_format.paragraph_get_line_spacing', return_value=1.5):
                    ...
                    with patch('wordformat.style.check_format.paragraph_get_first_line_indent', return_value=0):
                        # 模拟paragraph_get_builtin_style_name
                        with patch('wordformat.style.check_format.paragraph_get_builtin_style_name', return_value="正文"):
                            # 执行方法
...
def test_character_style_apply_to_run():
    """测试CharacterStyle的apply_to_run方法"""
    # 创建CharacterStyle实例
    char_style = CharacterStyle(
        font_name_cn="宋体",
    ...
    with patch('wordformat.style.check_format.run_get_font_size_pt', return_value=12.0):
        ...
        with patch('wordformat.style.check_format.run_get_font_color', return_value=(0, 0, 0)):
            ...
            with patch('wordformat.style.check_format.run_get_font_name', return_value="宋体"):
                # 模拟format方法
                with patch('wordformat.style.check_format.Alignment.format') as mock_alignment_format:
                    with patch('wordformat.style.check_format.FontSize.format') as mock_font_size_format:
                        with patch('wordformat.style.check_format.FontColor.format') as mock_font_color_format:
                            with patch('wordformat.style.check_format.FontName.format') as mock_font_name_format:
                                # 执行方法
...
def test_paragraph_style_apply_to_paragraph():
    """测试ParagraphStyle的apply_to_paragraph方法"""
    # 创建ParagraphStyle实例
    para_style = ParagraphStyle()
    ...
    # 模拟diff_from_paragraph
    with patch('wordformat.style.check_format.ParagraphStyle.diff_from_paragraph') as mock_diff_from:
        # 创建模拟的DIFFResult
        mock_diff1 = MagicMock()
        mock_diff1.diff_type = "alignment"
        mock_diff1.expected_value = "左对齐"
        mock_diff1.current_value = "居中"
        mock_diff1.comment = "期待左对齐"
        
        mock_diff2 = MagicMock()
        mock_diff2.diff_type = "space_before"
        mock_diff2.expected_value = "0.5行"
        mock_diff2.current_value = "1行"
        mock_diff2.comment = "期待0.5行"
        
        mock_diff_from.return_value = [mock_diff1, mock_diff2]
        
        # 模拟format方法
        with patch('wordformat.style.check_format.Alignment.format') as mock_alignment_format:
            with patch('wordformat.style.check_format.Spacing.format') as mock_spacing_format:
                # 执行方法
...
# 测试get_some.py中的内部辅助函数
def test_get_effective_line_height():
    """测试获取有效行高"""
    # 模拟段落对象
    paragraph = MagicMock()
    paragraph.paragraph_format = MagicMock()
    ...
    paragraph.style.font = MagicMock()
    paragraph.style.font.size = None
    assert _get_effective_line_height(paragraph) == 12.0
...
def test_get_style_spacing():
    ...
    assert _get_style_spacing(style, "before") == (0.0, 0)


def test_get_space_from_style():
    """测试从样式中获取间距设置"""
    # 模拟段落和样式对象
    paragraph = MagicMock()
    paragraph.style = MagicMock()
    ...
    # 测试无间距设置的情况
    paragraph.style.paragraph_format = MagicMock()
    paragraph.style.paragraph_format.space_before = None
    assert _get_space_from_style(paragraph, "before") == 0.0
...
def test_get_effective_line_height():
    ...
    paragraph.runs = [run]
    assert _get_effective_line_height(paragraph) == 14.0


def test_get_style_spacing():
    """测试从样式中获取间距"""
    # 测试无样式的情况
    assert _get_style_spacing(None, "before") == (0.0, 0)
...
def test_paragraph_get_line_spacing():
    ...
    paragraph.paragraph_format.line_spacing = None
    paragraph.style.paragraph_format = MagicMock()
    paragraph.style.paragraph_format.line_spacing_rule = None
    paragraph.style.paragraph_format.line_spacing = None
    assert paragraph_get_line_spacing(paragraph) == 1.0


def test_get_font_size_pt():
    """测试获取字体大小"""
    # 模拟段落对象
    paragraph = MagicMock()
    paragraph.runs = []
    ...
    # 测试无runs的情况
    assert _get_font_size_pt(paragraph) == 12.0
    
    # 测试有runs的情况
    run1 = MagicMock()
    run1.font = MagicMock()
    run1.font.size = MagicMock()
    run1.font.size.pt = 12.0
    
    run2 = MagicMock()
    run2.font = MagicMock()
    run2.font.size = MagicMock()
    run2.font.size.pt = 14.0
    
    paragraph.runs = [run1, run2]
    assert _get_font_size_pt(paragraph) == 14.0
...
def test_run_get_font_bold():
    ...
    # 测试font.bold为False的情况
    assert run_get_font_bold(run) == False
...
def test_run_get_font_italic():
    ...
    # 测试font.italic为False的情况
    assert run_get_font_italic(run) == False
...
def test_run_get_font_underline():
    ...
    # 测试font.underline为False的情况
    assert run_get_font_underline(run) == False
