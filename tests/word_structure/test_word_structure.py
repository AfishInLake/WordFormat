...
class TestDocumentTreeBuilder:
    ...
    def test_attach_heading_node(self):
        ...
        doc = Document()
        real_paragraph = doc.add_paragraph("测试段落")
        test_node = FormatNode(value={'category': 'test', 'paragraph': real_paragraph, 'fingerprint': 'test_fingerprint'}, expected_rule={}, level=1)
    ...
    def test_attach_body_node(self):
        ...
        doc = Document()
        real_paragraph = doc.add_paragraph("测试段落")
        test_node = FormatNode(value={'category': 'test', 'paragraph': real_paragraph, 'fingerprint': 'test_fingerprint'}, expected_rule={}, level=2)
...
class TestUtils:
    ...
    def test_promote_bodytext_in_subtrees_of_type(self):
        ...
        # 创建真实的Document和Paragraph对象
        from docx import Document
        doc = Document()
        real_paragraph = doc.add_paragraph("测试段落")

        body_node = BodyText(value={'category': 'body', 'paragraph': real_paragraph, 'fingerprint': 'body_fingerprint'}, expected_rule={}, level=2)
