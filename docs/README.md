# WordFormat 文档目录

本文档目录包含 WordFormat 项目的详细文档，按功能和使用场景分类组织。

## 文档索引

- [项目简介](https://github.com/AfishInLake/WordFormat/blob/master/docs/README.md) - 项目概述、功能特性和技术架构
- [安装指南](https://github.com/AfishInLake/WordFormat/blob/master/docs/installation.md) - 环境要求和安装步骤
- [使用指南](https://github.com/AfishInLake/WordFormat/blob/master/docs/usage.md) - 命令行、Python编程和API调用的详细使用方法
- [配置文件说明](https://github.com/AfishInLake/WordFormat/blob/master/docs/configuration.md) - 格式规范配置项和自定义配置方法
- [常见问题](https://github.com/AfishInLake/WordFormat/blob/master/docs/faq.md) - 常见问题及解决方案
- [贡献指南](https://github.com/AfishInLake/WordFormat/blob/master/docs/contributing.md) - 如何为项目贡献代码和文档
- [技术架构](https://github.com/AfishInLake/WordFormat/blob/master/docs/architecture.md) - 项目的技术架构和实现原理

## 快速导航

### 核心功能
- [智能文档结构解析](https://github.com/AfishInLake/WordFormat/blob/master/README.md#核心能力)
- [精细化格式校验](https://github.com/AfishInLake/WordFormat/blob/master/README.md#核心能力)
- [多级别标题管理](https://github.com/AfishInLake/WordFormat/blob/master/README.md#核心能力)
- [多语言适配](https://github.com/AfishInLake/WordFormat/blob/master/README.md#核心能力)
- [自动批注生成](https://github.com/AfishInLake/WordFormat/blob/master/README.md#实用功能)
- [格式一键修正](https://github.com/AfishInLake/WordFormat/blob/master/README.md#实用功能)

### 常用命令
- [生成文档结构 JSON](https://github.com/AfishInLake/WordFormat/blob/master/docs/usage.md#1-生成文档结构-json)
- [查看文档结构树](https://github.com/AfishInLake/WordFormat/blob/master/docs/usage.md#2-查看文档结构树)
- [执行格式校验](https://github.com/AfishInLake/WordFormat/blob/master/docs/usage.md#3-执行格式校验)
- [执行格式格式化](https://github.com/AfishInLake/WordFormat/blob/master/docs/usage.md#4-执行格式格式化)

### 配置指南
- [配置文件格式](https://github.com/AfishInLake/WordFormat/blob/master/docs/configuration.md#配置文件格式)
- [自定义配置](https://github.com/AfishInLake/WordFormat/blob/master/docs/configuration.md#自定义配置)
- [示例配置模板](https://github.com/AfishInLake/WordFormat/blob/master/docs/configuration.md#示例配置模板)

## 版本说明

文档版本与项目版本保持同步，如有任何疑问或建议，请通过 [GitHub Issues](https://github.com/AfishInLake/WordFormat/issues) 反馈。
